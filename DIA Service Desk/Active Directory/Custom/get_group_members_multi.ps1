...
# Title and AD groups needing to be searched
$title = "All Power BI Licenses"
$ad_groups = "lic-dia-ent-powerbi free", "lic-dia-ent-e5-powerbi pro", "lic-dia-ent-powerbi-premium"

# Format CSV fields
$fields = "ID`tName`tEmail`tUsername`tType`tActive`tLast Activity`tGroup`n"
...
foreach ($ad_group in $ad_groups) {
	...
	foreach ($user in $members_user) {
		...
		$results = "`t$($user.Name)`t$($user.EmailAddress)`t$($user.SAMAccountName)`t$($user.ObjectClass)`t$($user.Enabled)`t$($last_logon)`t$($ad_group)`n"
	...
	if ($members_other.count -gt 0) {
		foreach ($other in $members_other) {
			...
			# Format object properties into CSV record
			$results = "`t$($other.Name)`t`t`t$($other.ObjectClass)`t`t$($last_logon)`t$($ad_group)`n"
			# Add to the CSV for this group
			$members_string = $members_string + $count + $results
			$count ++
			# Add to the master CSV
