...
Title:   get_users_by_company.ps1
Author:  kian.mortimer@datacom.com
Date:    23/01/25
Version: 1.0

Description:
- Script to get the users from a specific company (much like department)

Workflow:
- Run this script and enter in the name of the company you wish to get the users of
...
# Change window title
$host.ui.RawUI.WindowTitle = "Get Users By Attribute - Company"

# Print instructions to output
Write-Host "* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *"
Write-Host "* Get users of a specific Company in AD                       *"
Write-Host "* Enter the name of the company into the prompt below         *"
...
:mainLoop while ($true) {
	...
	if ($first) {
		...
		Write-Host  " Enter Company: $($user_input_substring)"
	} else {
		# If it's not the first loop; get the user input normally
		$user_input = Read-Host " Enter Company" | ForEach-Object -Process { if ($_) { $_.Trim() } }
	}

    Write-Host "`n > Searching AD..."
	
	# Try-catch to make sure that an appropriate company was given
	try {
		# Search for users in AD with the specified Company field
        $members = Get-ADUser -Filter "Company -like ""$($user_input)""" -Properties *
		...
		Write-Host " ! Company `"$user_input`" not found.`n"
		continue
	}
	...
	if (!$members) {
		...
		Write-Host " ! AD Company `"$user_input`" has no members.`n"
        continue
	} else {
	...
	# Export to CSV
	$results | Export-Csv -Path ".\Exported Member Lists\Company $($user_input) $(Get-Date -Format 'dd-MM-yy').csv" -NoTypeInformation
	Write-Host " >>> Exported Member Lists\Company $($user_input) $(Get-Date -Format 'dd-MM-yy').csv"
